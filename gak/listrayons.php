<?PHP // $Id: listrayons.php,v 1.9 2014/06/16 12:00:06 shtifanov Exp $
    ...
    switch ($action)    {
        ...
        case 'oneenroll':
       	                $rez = one_enrol_teacher ($rid, $mid, $levelmonit, $typeou, $action);
                        redirect("listrayons.php?mid=$mid&rid=$rid&level=$levelmonit&typeou=$typeou", "Подписка учителей в тестах для района №{$rid} выполнена.", 5);
        break;
        
        case 'syncedutype':
                        $sql = "update mdl_monit_att_staff s inner join mdl_monit_college c on c.id=s.collegeid
                                set s.edutypeid=c.typeinstitution
                                where c.typeinstitution>0";
                        execute_sql($sql);        
                
                        $sql = "update mdl_monit_att_staff s inner join mdl_monit_school c on c.id=s.schoolid  
                        set s.edutypeid=c.typeinstitution
                        where s.schoolid not in (5776, 5180, 4585, 3990, 3385, 2769, 2116)";
                        execute_sql($sql);
                        
                        $sql = "update mdl_monit_att_staff s inner join mdl_monit_education c on c.id=s.douid  
                        set s.edutypeid=c.typeinstitution
                        where c.typeinstitution>0";
                        execute_sql($sql);
                        
                        $sql = "update mdl_monit_att_staff s inner join mdl_monit_udod c on c.id=s.udodid  
                        set s.edutypeid=c.typeinstitution
                        where c.typeinstitution>0";
                        execute_sql($sql);
        break;                        
    ...
    //	print_heading($strrayons, 'center', 3);
        ...
        echo '<br><br><div align="center">';
     	$options = array('rid' => $rid, 'mid' => $mid, 'level' => $levelmonit, 'typeou' => $typeou, 'action' => 'syncedutype');
     	print_single_button("listrayons.php", $options, 'Синхронизировать тип ОУ в карточках сотрудников');
        echo '</div>';
	...
	if ($allrayons)	 {
		...
		foreach ($allrayons as $rayon) 	{
			    ...
			    if($staffs = get_records_sql($strsql))   {
                     ...
                     $linkenrol = "<a title=\"Подписать данный район на тесты\"  href=\"listrayons.php?mid=$mid&rid={$rayon->id}&level=$levelmonit&typeou=$typeou&action=oneenroll\"><b><i>" . $rayon->number . '!!! </i></b></a>';
                     
       				 $table->data[] = array ($linkenrol.'.',
...
function enrol_teacher ($mid, $levelmonit, $typeou, $action)
{
    ...
    $ctxcatbase = get_context_instance (CONTEXT_COURSECAT, 9); // $outype->category  
    $roleid = 5; // student
...
function one_enrol_teacher ($rid, $mid, $levelmonit, $typeou, $action)
{
    global $CFG;
    ...
    $roleid = 5; // student
     ...
     $strsql = "SELECT concat(s.id, a.id), s.userid FROM mdl_monit_att_staff s 
                INNER JOIN mdl_monit_att_appointment a ON s.id=a.staffid
                WHERE rayonid=$rid and meetingid=$mid and edutypeid = $outype->id"; 
                // WHERE rayonid=$rayon->id and meetingid=$mid and $idou > 0";  
     if($staffs = get_records_sql($strsql))   {
        foreach ($staffs as $staff)    {
            switch ($action) {
