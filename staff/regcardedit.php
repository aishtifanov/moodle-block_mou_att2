...
<?php
    ...
    $appointments = get_records_select ('monit_att_appointment', "staffid={$user->staffid}", 'id DESC', 'id, staffid, stafftypeid, meetingid, appointment, pedagog_time, standing, standing_this, qualify, qualify_date, place_advan_train, date_advan_train, qualifynow, total_mark');
    if ($appointments)  {
        ...
        $profile->fields = array('appointment', 'standing_this', 'qualify', 'qualify_date', 'place_advan_train', 'date_advan_train', 'qualifynow'); // , 'stafftypeid', 'meetingid');
        $profile->type   = array('text', 'real', 'text', 'date', 'text', 'date', 'text'); // , 'stft', 'meet');
        $i=1;
